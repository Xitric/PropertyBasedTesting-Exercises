...
```
Here, the `@@` is replaced with a filename as chosen by AFL. For instance, files that AFL has generated by mutating the seed corpus.

Test cases that make ministat crash are located in `out/crashes/`. These test cases can be minimized with (using one of the failing test cases):
```
afl-tmin -i out/crashes/id:000001,sig:11,src:000027,op:arith8,pos:23,val:+18 -o in/crash-case ./ministat @@
```

The test case is then reduced to the following minimal test case found under `in/crash-case`:
```
0
0
9E700
